...
async.forEach(confArray, function(conf, callback){
    ...
    // S3 confs
    if(conf.assets.S3.access_key && conf.assets.S3.secret && conf.assets.S3.bucket && process.env.S3_ACCESS_KEY && process.env.S3_SECRET){
        conf.assets.S3.access_key = process.env.S3_ACCESS_KEY;
