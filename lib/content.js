...
module.exports = function(requestParam){
                        ...
                        if(item.path.substring(0, 1)!== '.'){
                            if(item.type === 'file'){
                                ...
                                if(item.path.substring(item.path.length - 9) === '.markdown' || item.path.substring(item.path.length - 3) === '.md'){
                                    // skip readme
                                    if(item.path !== ("README.markdown" || "README.md" || "Readme.markdown" || "Readme.md" || "readme.markdown" || "readme.md")){
    ...
    function parseContent(rawContent, callback){
        ...
        if(match){
            ...
            parsedObj.content = converter.makeHtml(rawContent.body.replace(match[0], '')).replace(idRegex, "$1 id=" + '"' + "$4" + '"' + "$2$3$5").replace(prettyprintRegexArr[0], '<pre class="prettyprint">$1').replace(prettyprintRegexArr[1], '<pre class="prettyprint linenums">$1' );
