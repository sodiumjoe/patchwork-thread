                              ...
                              function(err, results){
                                  ...
                                  async.forEach(results, 
                                      function(item, callback){
                                          options.fileFunc(item, callback);
                                      },
                                      callback);
                          });
                      }
                    , function(callback){
                          filterDirs(data, conf, options.filterDirs, 
                              function(err, results){
                                  async.forEach(results, 
                                      function(item, callback){
                                          if(options.dirFunc){
                                              options.dirFunc(item, conf, options, callback);
                                          }else{
                                              console.log(item.path);
                                              parseDir(item.path, conf, options, callback);
                                          }
                                      },
                                      callback);
                          });
